...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    val stringCounter = mutableMapOf<String, Int>()
    val inputString = File(inputName).bufferedReader().readText()
    for (i in substrings.indices) {
        stringCounter[substrings[i]] = 0
        for (k in inputString.indices) {
            if (inputString.startsWith(substrings[i], k, ignoreCase = true)) {
                stringCounter[substrings[i]] = stringCounter.getValue(substrings[i]) + 1
            }
        }
    }
...
fun chooseLongestChaoticWord(inputName: String, outputName: String) {
    ...
    outputWriter.write(wordsList.joinToString())
    outputWriter.close()
}
